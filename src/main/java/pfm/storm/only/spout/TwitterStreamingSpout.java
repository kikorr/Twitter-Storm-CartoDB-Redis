...
import pfm.storm.only.util.TwitterUtil;
...
public class TwitterStreamingSpout extends BaseRichSpout implements StatusListener {
	...
	public void onStatus(Status status) {
		...
		Tweet tweet = new Tweet(status.getId(), status.getCreatedAt(), status.getUser().getName(), 
				status.getText(), hashtags, status.getGeoLocation().getLongitude(),status.getGeoLocation().getLatitude());
		queue.offer(tweet);
	}
